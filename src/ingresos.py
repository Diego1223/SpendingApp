...
class Semana_Ingreso:
    def __init__(self, app, ingreso, n_semana):
        ...
        self.detalle_column = ft.Column(
            ...
            controls=[
                ft.Text(
                    f"Pago semanal: {self.ingreso}",
                    color=self.app.container2_color
                ),
                ft.Text(
                    f"Pago horas extras: {self.ingreso - 7200 if self.mostrar_extras else 'No hubo'}",
                    color=self.app.container2_color
                ),
                ft.Text(
                    f"Horas trabajadas: {round(self.ingreso / (7200 / 48))}",
                    color=self.app.container2_color
                ),
                ft.Text(
                    f"Horas extras: {round(self.ingreso / (7200 / 48)) - 48 if self.mostrar_extras else 'No hubo'}",
                    color=self.app.container2_color
                )
            ]
        )
                    ...
                    ft.Row(
                        ...
                        controls=[
                            ...
                            ft.IconButton(
                                ...
                                icon_color="white",
                                on_click=self.mostrar_calendario
                            )
                        ]
                    ),
                    self.detalle_column
...
class Clase_ingresos:
    ...
    def CambiarValores_btn(self):
        ...
        self.app.btn_ingresos.style = ft.ButtonStyle(
            side=ft.BorderSide(0.4, "white"),
            shape=ft.RoundedRectangleBorder(radius=10)
        )

        # Restablecer colores de Gastos
        self.app.btn_gastos.color = self.app.container2_color
        self.app.btn_gastos.bgcolor = self.app.container_color
        self.app.btn_gastos.style = ft.ButtonStyle(
            side=ft.BorderSide(1, self.app.container2_color),
            shape=ft.RoundedRectangleBorder(radius=10)
        )
    ...
    def Ingresos(self, e=None):
        ...
        ingresos_lista = [
            self.ingreso_semana1,
            self.ingreso_semana2,
            self.ingreso_semana3,
            self.ingreso_semana4
        ]
        ...
        # Crear DatePicker una sola vez
        if not hasattr(self.app, "date_picker"):
            self.app.date_picker = ft.DatePicker(
